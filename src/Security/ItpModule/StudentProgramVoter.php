...
class StudentProgramVoter extends CachedVoter
{
    public const MANAGE = 'ITP_STUDENT_PROGRAM_MANAGE';
    public const ACCESS = 'ITP_STUDENT_PROGRAM_ACCESS';
    ...
    {
        if (!$subject instanceof StudentProgram) {
            return false;
        }
    ...
    final public function voteOnAttribute($attribute, $subject, TokenInterface $token): bool
    {
        ...
        $training = $subject->getProgramGroup()?->getProgramGrade()?->getTrainingProgram()
            ?->getTraining();

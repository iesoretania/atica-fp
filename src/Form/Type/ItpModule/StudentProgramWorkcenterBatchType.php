...
class StudentProgramWorkcenterBatchType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder->addEventListener(FormEvents::PRE_SET_DATA, function (FormEvent $event) use ($options): void {
            ...
            $programGroup = $options['program_group'];
            ...
            $companies = $this->itpCompanyRepository->findByProgramGrade($programGrade);
            $teachers = $this->teacherRepository->findByGroup($programGroup->getGroup());
            $students = $this->studentEnrollmentRepository->findByGroup($programGroup->getGroup());
            ...
            $this->addElements($form, $companies, $workcenters, $teachers, $activities, $students);
        });
        ...
        $builder->addEventListener(FormEvents::PRE_SUBMIT, function (FormEvent $event) use ($options): void {
            ...
            $companies = $this->itpCompanyRepository->findByProgramGrade($programGroup->getProgramGrade());
            $teachers = $this->teacherRepository->findByGroup($programGroup->getGroup());
            $students = $this->studentEnrollmentRepository->findByGroup($group);

            if ($company instanceof Company) {
                $activities = $this->activityRepository->findByProgramGradeAndCompany($programGroup->getProgramGrade(), $company);
    ...
    private function addElements(FormInterface $form, array $companies, array $workcenters, array $teachers, array $activities, array $students): void
    {
        $form
            ->add('students', ChoiceType::class, [
                ...
                'choice_label' => function (StudentEnrollment $studentEnrollment) {
                    return $studentEnrollment->__toString();
                },
                'constraints' => [

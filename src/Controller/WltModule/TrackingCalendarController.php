...
use App\Form\Type\WltModule\WorkDayTrackingType;
...
use App\Security\WltModule\WorkDayVoter;
...
class TrackingCalendarController extends AbstractController
{
    ...
    public function attendanceReport(
        ...
        try {
            ...
            $title = $translator->trans('title.attendance', [], 'wlt_report')
                . ' - ' . $agreement->getStudentEnrollment()->__toString() . ' - '
                . $agreement->getWorkcenter()->__toString();
    ...
    ): Response
    {
        ...
        try {
            ...
            $this->pdfWriteFixedPosHTML($mpdf, $agreement->getWorkcenter()->__toString(), 192, 40.8, 72, 5);
            $this->pdfWriteFixedPosHTML($mpdf, $agreement->getProject()->getOrganization()->__toString(), 62.7, 40.9, 80, 5);
            $this->pdfWriteFixedPosHTML($mpdf, $agreement->getEducationalTutor()->__toString(), 97.5, 46.5, 46, 5);
            $this->pdfWriteFixedPosHTML($mpdf, $agreement->getWorkTutor()->__toString(), 198, 46.5, 66, 5);
            $this->pdfWriteFixedPosHTML(
                $mpdf,
                $agreement->getStudentEnrollment()->getGroup()->getGrade()->getTraining()->__toString(),
                172,
                54,
                61,
                5
            );
            $this->pdfWriteFixedPosHTML($mpdf, $agreement->getStudentEnrollment()->getPerson()->__toString(), 63, 54, 80, 5);
            ...
            $this->pdfWriteFixedPosHTML(
                $mpdf,
                $agreement->getStudentEnrollment()->getPerson()->__toString(),
                68,
                185.4,
                53,
                5
            );
            $this->pdfWriteFixedPosHTML($mpdf, $agreement->getEducationalTutor()->__toString(), 136, 186.9, 53, 5);
            $this->pdfWriteFixedPosHTML($mpdf, $agreement->getWorkTutor()->__toString(), 204, 184.9, 53, 5);
            ...
            $title = $translator->trans('title.weekly_activities', [], 'wlt_report')
                . ' - ' . $weekCounter['current'] . ' - ' . $agreement->getStudentEnrollment()->__toString() . ' - '
                . $agreement->getWorkcenter()->__toString();

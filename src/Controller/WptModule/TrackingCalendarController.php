...
use App\Entity\WptModule\ActivityTracking;
use App\Entity\WptModule\AgreementEnrollment;
use App\Entity\WptModule\TrackedWorkDay;
use App\Entity\WptModule\WorkDay;
use App\Form\Type\WptModule\WorkDayTrackingType;
...
class TrackingCalendarController extends AbstractController
{
    ...
    public function activitySummaryReport(
        ...
        ActivityTrackingRepository    $activityTrackingRepository,
        ActivityRepository            $activityRepository,
        TeacherRepository             $wptTeacherRepository,
        WorkDayRepository             $workDayRepository,
        TrackedWorkDayRepository      $trackedWorkDayRepository,
        AgreementEnrollment           $agreementEnrollment
    ): Response {
    ...
    ): Response
    {
        ...
        try {
            ...
            self::pdfWriteFixedPosHTML($mpdf, $agreementEnrollment->getEducationalTutor()->__toString(), 97.5, 46.5, 46, 5);
            self::pdfWriteFixedPosHTML($mpdf, $agreementEnrollment->getWorkTutor()->__toString(), 198, 46.5, 66, 5);
            self::pdfWriteFixedPosHTML(
                $mpdf,
                $agreementEnrollment->getStudentEnrollment()->getGroup()->getGrade()->getTraining()->__toString(),
                172,
                54,
                61,
                5
            ...
            self::pdfWriteFixedPosHTML(
                $mpdf,
                $agreementEnrollment->getStudentEnrollment()->getPerson()->__toString(),
                63,
                54,
                80,
                5
            ...
            self::pdfWriteFixedPosHTML(
                $mpdf,
                $agreementEnrollment->getEducationalTutor()->__toString(),
                136,
                186.9,
                53,
                5
            ...
            self::pdfWriteFixedPosHTML(
                $mpdf,
                $agreementEnrollment->getWorkTutor()->__toString(),
                204,
                184.9,
                53,
                5
            ...
            $title = $translator->trans('title.weekly_activities', [], 'wpt_report')
                . ' - ' . $weekCounter['current'] . ' - ' . $agreementEnrollment->getStudentEnrollment()->__toString() . ' - '
                . $agreementEnrollment->getAgreement()->getWorkcenter()->__toString();
